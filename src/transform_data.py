...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
  ...
  logging.info(f"\n-> Colunas de weather normalizadas - {len(df_weather.columns)} colunas")
  return df
...
def normalize_datatime_columns(df: pd.DataFrame, columns_names:list[str]) -> pd.DataFrame:
...
def data_transformations():
  ...
  df = normalize_datatime_columns(df, columns_to_normalize_datetime)
